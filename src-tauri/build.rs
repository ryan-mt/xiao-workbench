...
mod tests {
    use super::{
        normalize_build_version, ticket03_source_fingerprint, ticket03_source_manifest,
        APP_COMMANDS,
    };
    ...
    #[test]
    fn xai_commands_are_in_the_tauri_acl() {
        let root = workspace_root();
        let permissions = fs::read_to_string(root.join("src-tauri/permissions/main.toml"))
            .expect("main Tauri permissions should be readable");
        for command in [
            "create_xai_codex_profile",
            "begin_xai_device_oauth",
            "poll_xai_device_oauth",
            ...
            assert!(
                ...
                "{permission} must be enabled for the main window"
            );
            assert!(
                root.join(format!(
                ...
                .is_file(),
                "{command} must have a generated Tauri permission"
            );
        }
    }
}
